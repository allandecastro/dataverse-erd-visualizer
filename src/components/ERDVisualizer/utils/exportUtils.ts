...
// Constants for entity card dimensions (matching RelationshipLines.tsx)
...
export async function copyToClipboardAsPNG(options: ExportOptions): Promise<void> {
  ...
  entities.forEach(entity => {
    ...
    if (!isCollapsed && visibleFieldCount > 0) {
      const cardHeight = HEADER_HEIGHT + ATTRIBUTES_TITLE_HEIGHT + (visibleFieldCount * FIELD_HEIGHT) + 20;
  ...
  relationships.forEach(rel => {
    ...
    const start = getConnectionPoint(entities, entityPositions, selectedFields, collapsedEntities, rel.from, lookupField?.name, startSide);
    const end = getConnectionPoint(entities, entityPositions, selectedFields, collapsedEntities, rel.to, toEntity.primaryIdAttribute, endSide);
    ...
    ctx.bezierCurveTo(
      adjStart.x + curveStrength, adjStart.y,
      adjEnd.x - curveStrength, adjEnd.y,
      adjEnd.x, adjEnd.y
    );
    ctx.stroke();
  ...
  entities.forEach(entity => {
    ...
    const cardHeight = isCollapsed || visibleFields.length === 0
      ? 80
    ...
    if (!isCollapsed && visibleFields.length > 0) {
      ...
      visibleFields.forEach(attr => {
        ...
        ctx.fillText(attr.displayName.substring(0, 25), x + (attr.isPrimaryKey ? 36 : 20), fieldY + 16);
...
export function exportToSVG(options: ExportOptions): string {
  ...
  relationships.forEach(rel => {
    ...
    // Draw cardinality markers
    svg.appendChild(createCardinalityMarkerSVG(
      svgNS,
      start.x,
      start.y,
      cardinality.from === '1' ? 'one' : 'many',
    ...
    svg.appendChild(createCardinalityLabelSVG(svgNS, startLabelX, start.y, cardinality.from, isDarkMode));
  ...
  entities.forEach(entity => {
    ...
    if (!isCollapsed && visibleFields.length > 0) {
      ...
      visibleFields.forEach(attr => {
        ...
        fieldBg.setAttribute("stroke", attr.isPrimaryKey ? "#fbbf24" : (isDarkMode ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"));
